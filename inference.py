...
# parser.add_argument("--img-height", default=384, type=int, help="Image height")
# parser.add_argument("--img-width", default=384, type=int, help="Image width")
...
#         img = imread(file).astype(np.float32)[:,:,:3]
...
def main():
    ...
    disp_net = DispResNet(args.resnet_layers, False).to(device)
    weights = torch.load(args.pretrained, map_location=device)  # map_location argument added
    disp_net.load_state_dict(weights['state_dict'])
    disp_net.eval()
